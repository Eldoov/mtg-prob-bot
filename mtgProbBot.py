...
def parse_input(content: str):
    ...
    try:
        ...
        raise ValueError(
            "Invalid input. Ensure the input is two groups of integers separated by a comma. "
...
def validate_input(deck_nums, want_nums):
    ...
    """
    if sum(deck_nums) > 99:
        raise ValueError("The sum of the first group (deck composition) cannot exceed 99.")
    if sum(want_nums) > 7:
        raise ValueError("The sum of the second group (desired draws) cannot exceed 7.")
...
async def help_command(ctx):
    ...
    """
    help_text = (
        "This bot calculates the probability of drawing specific cards in a MTG commander game.\n\n"
        "### Example\n"
        "`!prob 37 15 20, 3 1 2`\n"
    ...
    try:
        ...
        if detailed:
            ...
            if len(full_res) <= max_length:
                await ctx.send(full_res)
            else:
                # Send detailed results in chunks
                for i in range(0, len(full_res), max_length):
                    await ctx.send(full_res[i:i + max_length])
        else:
            await ctx.send(f"The probability of given data is: {simp_res}")
